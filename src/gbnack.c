...
int get_ack(GBNAck self, int socket_handler, struct sockaddr_in from) {
    ...
    if (bytes_rcvd > 0){
        ...
        memcpy(c_rev_win_size, buffer+4, 4);

        self->seq_num = atoi(c_seq_num);
        self->rev_win_size = atoi(c_rev_win_size);
    }
    return bytes_rcvd;   
}
